...
export default async (word, options) => {
  console.log('');
  const { iciba, groq, openai, GROQ_API_KEY } = options;
  ...
  // openai
  if (isTrueOrUndefined(openai) && options.openaiClient) {
    const model = 'gpt-4o-mini';
    const spinner = ora(`正在请教 ${model}...`).start();
    try { 
      const stream = await options.openaiClient.chat.completions.create({
        model: model,
        messages: [
      ...
      for await (const chunk of stream) {
        process.stdout.write(chunk.choices[0]?.delta?.content || "", false);
      }
    } catch (error) {
      spinner.fail(`访问 ${model} 失败，请检查网络或 API 密钥`);
    }
